...
partial class LlvmTextEmitter {
    ...
    // Beans source line tables. `--debug` asks for a binary a person can stop
    // inside, so that build writes LLVM debug metadata beside its
    // instructions and every other build writes none. src/llvm_debug.b holds
    // the whole of it.
    debug_info: bool
    debug_meta: List<string>
    debug_meta_ids: Map<string, int>
    debug_file_ids: Map<string, int>
    debug_type_ids: Map<string, int>
    ...
    fn init(program: MirProgram, mir_comments: bool,
        ...
        self.debug_main_file = ""
        self.debug_unit = -1
        self.debug_subroutine_type = -1
        self.debug_scope = -1
    ...
    fn emit(require_main: bool) -> string {
        ...
        // The unit and its file come first: a subprogram cannot name a unit
        // that has no id yet, and the first subprogram is minted as soon as
        // the first function body is emitted below.
        self.open_debug_unit()
        ...
        // Named metadata has to follow the triple. Above it, Clang stops with
        // "expected top-level entity" pointed at the triple line. The
        // definitions these reference are written at the end of the module.
        output = "{output}{self.module_named_metadata()}"
        output = "{output}{self.debug_module_declares()}"
        output =
        ...
        // of module_head or module_globals: a chunk that repeated it would
    ...
    fn chunk_modules(count: int) -> List<string> {
        ...
        // Debug metadata is one graph rooted at one compile unit, and a
