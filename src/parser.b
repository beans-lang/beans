...
class Parser {
    ...
    fn parse_annotations() -> List<AstNode> {
        var annotations: List<AstNode> = []
        for self.check("@") {
            let at: Token = self.advance()
            let first: Token =
                self.expect("ident", "expected annotation name after '@'")
            var name: string = first.text
            if self.match_token(".") {
                let member: Token =
                    self.expect("ident", "expected annotation name after '.'")
                name = "{name}.{member.text}"
            }
            let annotation: AstNode =
                self.named(self.node("annotation", name, at), first)
            if self.match_token("(") {
                self.skip_newlines()
                for !self.check(")") && !self.at_end() {
                    let argument_name: Token =
                        self.expect("ident", "expected annotation argument name")
                    ...
                    self.expect(
                        ":",
                        "annotation arguments are named, like value: 1")
                    argument.add(self.parse_expression())
                    annotation.add(argument)
                    if !self.match_token(",") { break }
                    self.skip_newlines()
                }
                self.expect(")", "expected ')' after annotation arguments")
            }
    ...
    fn parse_declaration_body() -> AstNode {
        ...
        if self.check("ident") &&
           self.current().text == "annotation" &&
           self.tokens[self.pos + 1].kind == "ident" {
            let result: AstNode = self.parse_annotation_declaration()
        ...
        // Class-only modifiers stay contextual so ordinary declarations may
        // keep using these words as names. Accept the whole modifier run here
        // and let HIR validation report invalid combinations precisely.
        if self.check("ident") &&
           (self.current().text == "unique" ||
            self.current().text == "abstract" ||
            self.current().text == "singleton" ||
            self.current().text == "partial") {
            let first: Token = self.current()
            var modifiers: string = ""
            for self.check("ident") &&
                (self.current().text == "unique" ||
                 self.current().text == "abstract" ||
                 self.current().text == "singleton" ||
                 self.current().text == "partial") {
                ...
                modifiers =
                    if modifiers == "" { part } else { "{modifiers} {part}" }
            }
            if self.check("class") {
                let result: AstNode = self.parse_type_declaration()
                result.value = "{modifiers} {result.value}"
                if public { result.value = "pub {result.value}" }
                return result
            }
    ...
    fn parse_annotation_declaration() -> AstNode {
        let start: Token = self.advance()
        let name: Token =
            self.expect("ident", "expected annotation name")
        let declaration: AstNode =
            self.named(
                self.node("annotation_decl", name.text, start),
                name)
        self.expect("\{", "expected '\{' after annotation name")
        self.skip_newlines()
        for !self.check("\}") && !self.at_end() {
            let field_name: Token =
                self.expect("ident", "expected annotation field name")
            let field: AstNode =
                self.node(
                    "annotation_field", field_name.text, field_name)
            self.expect(":", "expected ':' after annotation field name")
    ...
    fn parse_function() -> AstNode {
        ...
        for !self.check(")") && !self.at_end() {
            ...
            parameter.annotations = move annotations
            if passing != "" {
                parameter.add(
    ...
    fn parse_type_declaration() -> AstNode {
        ...
        for !self.check("\}") && !self.at_end() {
                   ...
                   self.check("pub") {
                    ...
                    if modifier == "" {
                        ...
                        modifier = "{modifier} {part}"
                          ...
                          self.current().text == "priv" &&
                          self.tokens[self.pos + 1].kind != ":" &&
                          self.tokens[self.pos + 1].kind != "=" {
                    // `priv` is contextual. A field may still be named
                    // `priv`, but `priv value: T` marks `value` as
                    // visible only inside its declaring type.
                    let part: string = self.advance().text
                    if modifier == "" {
                        modifier = part
                    } else {
            ...
            if self.check("fn") {
                ...
                method.annotations = move annotations
                ...
                field.note = modifier
                field.annotations = move annotations
                if self.match_token(":") {
                    field.add(self.parse_type())
                } else if self.match_token("(") {
                    self.skip_newlines()
                    for !self.check(")") && !self.at_end() {
                        let payload_annotations: List<AstNode> =
                            self.parse_annotations()
                        let payload: Token =
                            self.expect("ident", "expected payload name")
                        let payload_node: AstNode =
                            self.node("payload", payload.text, payload)
                        payload_node.annotations =
                            move payload_annotations
                        if self.match_token(":") {
    ...
    fn parse_for() -> AstNode {
        ...
        if self.check("ident") &&
           (self.tokens[self.pos + 1].kind == ":" ||
            ...
            result.add(binding)
            if self.match_token(",") {
                let value_name: Token =
                    self.expect("ident", "expected value binding")
                let value_binding: AstNode =
                    self.node("binding", value_name.text, value_name)
                if self.match_token(":") {
                    value_binding.add(self.parse_type())
                }
                result.add(value_binding)
            }
    ...
    fn parse_primary() -> AstNode {
        let token: Token = self.current()
        // Keep a closing delimiter in the stream when an operand is missing.
        // Its owner can recover normally instead of also reporting that the
        ...
        if token.kind == "newline" || token.kind == "\}" ||
           ...
           token.kind == "," || token.kind == "eof" {
            ...
            // delimiter in the stream for its owner.
