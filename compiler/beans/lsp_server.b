...
fn lsp_file_uri(file_path: string) -> string {
    var out: string = "file://"
    for index: int in 0..file_path.len() {
        let byte: int = file_path.byte_at(index)
        let unreserved: bool =
            (byte >= 48 && byte <= 57) ||
            (byte >= 65 && byte <= 90) ||
            (byte >= 97 && byte <= 122) ||
...
class BeansLspServer {
    ...
    fn full_range(text: string,
        ...
        if end_line < declaration.line {
            ...
            end_col = declaration.col
        }
        let start_source: string =
            lsp_line(text, declaration.line - 1)
        ...
        return lsp_object([
            lsp_member(
                ...
                lsp_position(
                    declaration.line - 1,
                    lsp_utf16_index(
                        start_source, declaration.col - 1))),
            lsp_member(
                "end",
                lsp_position(
                    end_line - 1,
    ...
    fn member_conflict(snapshot: SemanticSnapshot,
        ...
        for owner: string in
            ...
            for member: SemanticDecl in
                semantic_members(snapshot, owner_id, true) {
