...
struct Workload {
    ...
    bool scored = true;
    std::string comparison = "same-work";
};

struct Policy {
    int schema = 0;
    double max_cv_percent = 0.0;
    int max_noise_retries = 0;
    double tuned_overall_floor = 0.0;
    double tuned_group_floor = 0.0;
    double tuned_workload_floor = 0.0;
    double tuned_memory_ceiling = 0.0;
    double matched_overall_floor = 0.0;
    double matched_group_floor = 0.0;
    double matched_workload_floor = 0.0;
    double matched_memory_ceiling = 0.0;
    double compare_overall_regression = 0.0;
    double compare_group_regression = 0.0;
    double compare_workload_regression = 0.0;
    double compare_memory_regression = 0.0;
...
// Ownership traffic, read from a separate -DBEANS_ARC_STATS binary built from
...
struct ArcStats {
    ...
    std::uint64_t release_nodes = 0;
    std::uint64_t frees = 0;
    std::uint64_t possible_roots = 0;
    std::uint64_t collections = 0;
    std::uint64_t cycle_objects = 0;
};
    ...
    while (std::getline(input, line)) {
        ...
        if (fields.size() != 7 && fields.size() != 8)
            throw std::runtime_error("manifest line " + std::to_string(line_number) +
                                     " needs 7 or 8 tab-separated fields");
        rows.push_back({fields[0], fields[1], fields[2] == "1", fields[3],
                        fields[4], fields[5], fields[6] == "1",
                        fields.size() == 8 ? fields[7] : "same-work"});
    }
    return rows;
}

static Policy read_policy(const fs::path& path) {
    std::ifstream input(path);
    ...
    while (std::getline(input, line)) {
        ...
        auto fields = split_tabs(line);
        if (fields.size() != 2)
            throw std::runtime_error("policy line " + std::to_string(line_number) +
                                     " needs 2 tab-separated fields");
        if (values.contains(fields[0]))
            throw std::runtime_error("duplicate policy key: " + fields[0]);
        values[fields[0]] = std::stod(fields[1]);
    }
    auto required = [&](const std::string& key) {
        auto found = values.find(key);
        if (found == values.end())
            throw std::runtime_error("missing policy key: " + key);
        return found->second;
    };
    Policy p;
    p.schema = static_cast<int>(required("schema"));
    p.max_cv_percent = required("max_cv_percent");
    p.max_noise_retries =
        static_cast<int>(required("max_noise_retries"));
    p.tuned_overall_floor = required("tuned_overall_floor");
    p.tuned_group_floor = required("tuned_group_floor");
    p.tuned_workload_floor = required("tuned_workload_floor");
    p.tuned_memory_ceiling = required("tuned_memory_ceiling");
    p.matched_overall_floor = required("matched_overall_floor");
    p.matched_group_floor = required("matched_group_floor");
    p.matched_workload_floor = required("matched_workload_floor");
    p.matched_memory_ceiling = required("matched_memory_ceiling");
    p.compare_overall_regression = required("compare_overall_regression");
    p.compare_group_regression = required("compare_group_regression");
    p.compare_workload_regression = required("compare_workload_regression");
    p.compare_memory_regression = required("compare_memory_regression");
    p.expected_improvement = required("expected_improvement");
    if (p.schema != 1) throw std::runtime_error("unsupported policy schema");
    if (p.max_noise_retries < 0 || p.max_noise_retries > 8)
        throw std::runtime_error(
            "max_noise_retries must be between 0 and 8");
    return p;
}
...
static void fnv1a_append(std::uint64_t& hash, std::string_view text) {
    for (unsigned char byte : text) {
        hash ^= byte;
        hash *= 1099511628211ULL;
    }
...
static std::string file_hash(const fs::path& path) {
    ...
    while (input) {
        ...
        fnv1a_append(hash, std::string_view(buffer.data(),
                                            static_cast<std::size_t>(input.gcount())));
    }
    return hex64(hash);
}

static std::string source_tree_hash(const fs::path& root,
                                    const std::vector<Workload>& manifest) {
    std::set<std::string> workload_names;
    for (const auto& workload : manifest)
        workload_names.insert(workload.name);
    ...
    for (const auto& entry : fs::recursive_directory_iterator(root)) {
        ...
        const bool named_workload =
            workload_names.contains(entry.path().stem().string());
        const bool workload =
            named_workload &&
            (ext == ".b" || ext == ".go" || ext == ".js" ||
             (relative.starts_with("cpp/") && ext == ".cpp"));
        const bool shared_cpp_header =
            relative.starts_with("cpp/") && ext == ".h";
        if (workload || shared_cpp_header || relative == "suite.tsv")
    ...
    for (const auto& path : files) {
        ...
        fnv1a_append(hash, relative);
        fnv1a_append(hash, std::string_view("\0", 1));
        std::ifstream input(path, std::ios::binary);
        std::array<char, 64 * 1024> buffer{};
        while (input) {
            input.read(buffer.data(), static_cast<std::streamsize>(buffer.size()));
            fnv1a_append(hash, std::string_view(
                                     buffer.data(),
                                     static_cast<std::size_t>(input.gcount())));
        }
        fnv1a_append(hash, std::string_view("\0", 1));
...
static RunResult run_process(const std::vector<std::string>& command,
    ...
    // The ARC report goes to stderr; fold it into the same pipe so one read
    // gets both. Only the untimed counted run asks for this.
    if (capture_stderr)
...
// One untimed run of the counted binary, after all measurement is done.
static ArcStats read_arc_stats(const fs::path& binary, const Workload& workload,
                               std::string_view expected_output) {
    ArcStats stats;
    if (!fs::exists(binary)) return stats;
    const auto run = run_process({binary.string(), workload.size, workload.seed},
                                 true);
    const auto marker = run.output.find("beans arc stats:");
    if (marker == std::string::npos) return stats;
    // stdout is block-buffered through the pipe while the atexit report goes
    // straight out, so the report can land before the program's own output.
    // Compare with that one line lifted out rather than assuming an order.
    const auto line_end = run.output.find('\n', marker);
    const std::string without =
        run.output.substr(0, marker) +
        (line_end == std::string::npos ? std::string()
                                       : run.output.substr(line_end + 1));
    if (without != expected_output)
        throw std::runtime_error(workload.name +
                                 " ARC-counted build changed the output");
    const std::map<std::string, std::uint64_t*> fields = {
        {"allocations", &stats.allocations},
        {"allocated_bytes", &stats.allocated_bytes},
        {"retains", &stats.retains},
        {"releases", &stats.releases},
        {"release_nodes", &stats.release_nodes},
        {"frees", &stats.frees},
        {"possible_roots", &stats.possible_roots},
        {"collections", &stats.collections},
        {"cycle_objects", &stats.cycle_objects},
    };
    std::istringstream line(run.output.substr(marker));
    std::string token;
    while (line >> token) {
        const auto split = token.find('=');
        if (split == std::string::npos) continue;
        const auto found = fields.find(token.substr(0, split));
        if (found == fields.end()) continue;
        *found->second = std::strtoull(token.c_str() + split + 1, nullptr, 10);
        stats.present = true;
    }
    return stats;
}
...
static void write_json(const fs::path& path, std::string_view mode,
    ...
    out << "\n  },\n  \"policy\": {\n"
        ...
        << "    \"max_noise_retries\": "
        << policy.max_noise_retries << ",\n"
        << "    \"tuned_overall_floor\": " << policy.tuned_overall_floor << ",\n"
        << "    \"tuned_group_floor\": " << policy.tuned_group_floor << ",\n"
        << "    \"tuned_workload_floor\": " << policy.tuned_workload_floor << ",\n"
        << "    \"tuned_memory_ceiling\": " << policy.tuned_memory_ceiling << ",\n"
        << "    \"matched_overall_floor\": " << policy.matched_overall_floor << ",\n"
        << "    \"matched_group_floor\": " << policy.matched_group_floor << ",\n"
        << "    \"matched_workload_floor\": " << policy.matched_workload_floor << ",\n"
        << "    \"matched_memory_ceiling\": " << policy.matched_memory_ceiling << ",\n"
        << "    \"compare_overall_regression\": "
        << policy.compare_overall_regression << ",\n"
        << "    \"compare_group_regression\": " << policy.compare_group_regression
        << ",\n"
        << "    \"compare_workload_regression\": "
        << policy.compare_workload_regression << ",\n"
        << "    \"compare_memory_regression\": "
        << policy.compare_memory_regression << ",\n"
    ...
    for (std::size_t b = 0; b < results.size(); ++b) {
        ...
        out << "    {\n      \"name\": \"" << json_escape(bench.workload.name)
            ...
            << "\",\n      \"comparison\": \""
            << json_escape(bench.workload.comparison)
            << "\",\n      \"scored\": "
            << (bench.workload.scored ? "true" : "false")
        ...
        out << "      ]";
        if (bench.arc.present)
            out << ",\n      \"arc\": {\"allocations\": " << bench.arc.allocations
                << ", \"allocated_bytes\": " << bench.arc.allocated_bytes
                << ", \"retains\": " << bench.arc.retains
                << ", \"releases\": " << bench.arc.releases
                << ", \"release_nodes\": " << bench.arc.release_nodes
                << ", \"frees\": " << bench.arc.frees
                << ", \"possible_roots\": " << bench.arc.possible_roots
                << ", \"collections\": " << bench.arc.collections
                << ", \"cycle_objects\": " << bench.arc.cycle_objects << "}";
        out << "\n    }" << (b + 1 == results.size() ? "\n" : ",\n");
    }
    out << "  ]\n}\n";
}
...
static void write_report(const fs::path& path, std::string_view mode,
    ...
    for (const auto& [group, score] : scores) {
        (void)group;
        if (score.first < policy.tuned_group_floor) tuned_group_floor = false;
        if (score.second < policy.matched_group_floor) safe_group_floor = false;
    }
    bool tuned_workload_floor = true;
    bool safe_workload_floor = true;
    double weakest_tuned = 0.0;
    double weakest_matched = 0.0;
    std::string weakest_tuned_name;
    std::string weakest_matched_name;
    ...
    for (const auto& bench : results) {
        ...
        if (beans && tuned && matched) {
            ...
            if (weakest_tuned_name.empty() || tuned_score < weakest_tuned) {
                weakest_tuned = tuned_score;
                weakest_tuned_name = bench.workload.name;
            }
            if (weakest_matched_name.empty() || matched_score < weakest_matched) {
                weakest_matched = matched_score;
                weakest_matched_name = bench.workload.name;
            }
            if (tuned_score < policy.tuned_workload_floor)
                tuned_workload_floor = false;
            if (matched_score < policy.matched_workload_floor)
                safe_workload_floor = false;
        }
        for (const auto& target : bench.targets) {
            if ((target.id == "beans" || target.id == "cpp_tuned" ||
                 target.id == "cpp_matched") &&
                target.stats.cv_percent > policy.max_cv_percent) {
                unstable_rows += 1;
            }
        }
    }
    const auto dirty = metadata.find("dirty");
    const bool clean = dirty != metadata.end() && dirty->second == "false";
    const bool eligible =
        mode == "full" && missing.empty() && missing_contract.empty() &&
        unstable_rows == 0 && clean;

    out << "# Beans benchmark report\n\n";
    out << "- Mode: " << mode << "\n";
    for (const auto& key : {"date", "git", "dirty", "worktree_fnv1a64", "os",
                            "cpu", "logical_cpus", "memory_bytes", "cxx",
                            "cxx_flags", "beans_flags", "suite_fnv1a64",
                            "compiler_fnv1a64", "runtime_fnv1a64",
                            "policy_fnv1a64", "go", "bun", "noise_reason"}) {
        auto found = metadata.find(key);
        if (found != metadata.end()) out << "- " << key << ": " << found->second << "\n";
    }
    auto result_path = metadata.find("result_path");
    out << "- Raw data: `"
        << (result_path != metadata.end()
                ? result_path->second
                : "build/bench/results-" + std::string(mode) + ".json")
        << "`\n\n";
    out << "Cold starts are stored in JSON and excluded below. `!` means CV is above "
        << policy.max_cv_percent << "%. "
    ...
    out << "| workload | group | comparison | Beans | tuned C++ | matched C++ | vs tuned | vs matched | Beans RSS | tuned RSS | matched RSS |\n";
    out << "|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|\n";
    ...
    for (const auto& [group, score] : scores) {
        out << "| " << group << " | " << std::fixed << std::setprecision(1)
            ...
            << (score.first >= policy.tuned_group_floor ? "PASS" : "FAIL") << " | "
            << (score.second >= policy.matched_group_floor ? "PASS" : "FAIL")
            << " |\n";
    }
    ...
    const bool tuned_memory_pass =
        tuned_memory > 0.0 && tuned_memory <= policy.tuned_memory_ceiling;
    const bool safe_memory_pass =
        matched_memory > 0.0 && matched_memory <= policy.matched_memory_ceiling;
    const bool pass80 = eligible &&
                        matched_overall >= policy.matched_overall_floor &&
                        safe_group_floor && safe_workload_floor && safe_memory_pass;
    const bool pass90 = eligible && tuned_overall >= policy.tuned_overall_floor &&
                        tuned_group_floor && tuned_workload_floor &&
                        tuned_memory_pass;
    out << "Safe 80% target: **" << (pass80 ? "PASS" : "FAIL") << "**.  \n";
    out << "Peak-memory ratio: " << std::fixed << std::setprecision(2) << tuned_memory
        << "x tuned C++ (target " << policy.tuned_memory_ceiling << "x), "
        << matched_memory << "x matched C++ (safe target "
        << policy.matched_memory_ceiling << "x).  \n";
    out << "Weakest scored workload: " << weakest_tuned_name << ' '
        << std::setprecision(1) << weakest_tuned * 100.0 << "% tuned, "
        << weakest_matched_name << ' ' << weakest_matched * 100.0
        << "% matched.  \n";
    ...
    if (!eligible) {
        ...
        if (!missing_contract.empty())
            out << "; full contract still needs " << missing_contract.size()
                << " workloads";
        if (unstable_rows)
            out << "; " << unstable_rows << " required target row(s) exceed "
                << policy.max_cv_percent << "% CV";
        if (!clean) out << "; working tree is dirty";
    }
    out << ".\n";
    ...
    bool have_arc = false;
    for (const auto& bench : results)
        if (bench.arc.present) have_arc = true;
    if (have_arc) {
        out << "\n## Ownership traffic\n\n";
        out << "Counted by a separate `-DBEANS_ARC_STATS` build of the same emitted"
               " IR, run once outside the timed rounds, so the timed binaries never"
               " contain a counter. That build is not LTO'd, so it can keep"
               " retain/release pairs the shipping binary folds away — read these as"
               " the ownership work the compiler emitted.\n\n";
        out << "| workload | allocations | bytes | retains | releases | freed | cycle roots | collections | cycle objects |\n";
        out << "|---|---:|---:|---:|---:|---:|---:|---:|---:|\n";
        for (const auto& bench : results) {
            if (!bench.arc.present) continue;
            out << "| " << bench.workload.name << " | " << bench.arc.allocations
                << " | " << format_bytes(bench.arc.allocated_bytes) << " | "
                << bench.arc.retains << " | " << bench.arc.releases << " | "
                << bench.arc.frees << " | " << bench.arc.possible_roots << " | "
                << bench.arc.collections << " | " << bench.arc.cycle_objects
                << " |\n";
        }
    }
...
int main(int argc, char** argv) try {
    if (argc != 9) {
        std::cerr << "usage: bench-runner <quick|full|verify> <manifest> <bin-dir> "
                     "<compile-times> <json> <report> <repo-root> <policy>\n";
        return 2;
    }
    ...
    const Policy policy = read_policy(argv[8]);
    ...
    for (const auto& workload : manifest) {
        ...
        if (mode == "full" && workload.scored) {
            ...
            for (int retry = 1;
                 retry <= policy.max_noise_retries; ++retry) {
                bool unstable = false;
                for (auto index : required_order)
                    if (bench.targets[index].stats.cv_percent >
                        policy.max_cv_percent)
                        unstable = true;
                if (!unstable) break;
        ...
        if (mode == "full") {
            bench.arc = read_arc_stats(bin_dir / (workload.name + "_arc"),
                                       workload, bench.output);
            if (bench.arc.present) std::cout << ", arc" << std::flush;
        }
        std::cout << " — " << actual_hash << '\n';
    ...
    const auto git_status = command_output("git status --porcelain 2>/dev/null");
    metadata["dirty"] = git_status.empty() ? "false" : "true";
...
#endif
    const char* cxx_version = std::getenv("BENCH_CXX_VERSION");
    metadata["cxx"] = cxx_version ? cxx_version : "unknown";
    ...
    metadata["logical_cpus"] = std::to_string(std::thread::hardware_concurrency());
    const long pages = sysconf(_SC_PHYS_PAGES);
    const long page_size = sysconf(_SC_PAGESIZE);
    if (pages > 0 && page_size > 0)
        metadata["memory_bytes"] =
            std::to_string(static_cast<unsigned long long>(pages) *
                           static_cast<unsigned long long>(page_size));
    metadata["suite_fnv1a64"] = source_tree_hash(root / "bench", manifest);
    metadata["compiler_fnv1a64"] = file_hash(root / "build" / "beansc");
    metadata["runtime_fnv1a64"] = file_hash(root / "runtime" / "beans_rt.c");
    metadata["policy_fnv1a64"] = file_hash(argv[8]);
    metadata["result_path"] = fs::relative(json_path, root).generic_string();
    const auto worktree_state =
        git_status + "\n" +
        command_output("git diff --no-ext-diff --binary HEAD -- . 2>/dev/null");
    metadata["worktree_fnv1a64"] = hex64(fnv1a(worktree_state));
    ...
    write_json(json_path, mode, results, metadata, policy);
    write_report(report_path, mode, results, metadata, policy);
