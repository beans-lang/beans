...
fn main() {
    ...
    // Text goes between the file and its final string directly. There is no
    // full-size temporary Bytes on either read or write.
    ...
    // copy / rename / remove. fs.copy uses the OS file-copy path and only
    // needs a fixed-size buffer on systems which do not provide one.
