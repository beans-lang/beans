...
fn main() {
    ...
    io.println("{collections.sum_int(numbers)} {collections.count(numbers, 2)}")
    io.println("{counts["red"]} {counts["blue"]} {collections.unique(words)}")
    io.println("{math.clamp(12, 0, 10)} {math.gcd(84, 30)}")
    ...
    io.println("{collections.count(numbers, 2)} {collections.unique(numbers)} {labels}")

    var inventory: Map<string, int> = {"beans": 2, "rice": 4}
    let incremented: int = collections.increment(inout inventory, "beans", 3)
    let created: int = collections.get_or_insert_with(
        inout inventory, "tea", fn() -> int { return 7 })
    let existing: int = collections.get_or_insert_with(
    ...
    let inventory_text: Map<string, string> = collections.map_values_with_key(inventory,
    ...
    let encoded: Bytes = byte_algo.encode_uvarint(300)
    ...
    for value: u64 in boundaries {
        let item: Bytes = byte_algo.encode_uvarint(value)
    ...
    io.println("path {path.join("a/", "b")} {path.join("a", "/root")} {path.parent("/a/b/")} {path.name("/a/b/")} {path.extension("archive.tar.gz")} {path.stem("archive.tar.gz")} {path.extension(".bashrc")}")
