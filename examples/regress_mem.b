...
// A temporary collection is the loop's owner. Continue must not release it
// while the next iteration still reads it, and break must not release it twice.
fn temporary_loop_check() {
    var kept: string = ""
    for line: string in "\nkeep".lines() {
        if line == "" { continue }
        kept = line
    }
    for word: string in "stop,unused".split(",") {
        kept = "{kept}:{word}"
        break
    }
    io.println(kept)
