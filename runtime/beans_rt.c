...
void* beans_rt_host_symbol(const char* name) {
    ...
    // The non-zeroing allocator the typed JSON decoder hands to its bridge as
    // the string-payload callback. Exported here so the linker keeps it and so
    // a natively-compiled interpreter can reach it by name, like the entries
    // above.
    if (strcmp(name, "beans_alloc_bytes") == 0)
        return (void*)&beans_alloc_bytes;
    return (void*)0;
}
