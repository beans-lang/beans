...
// be padded, so a strict decode of an *unpadded* encoding pads a partial
// tail into a scratch copy and revalidates. That keeps simdutf's trailing-
// bits check while accepting the format's own shape.
BEANS_ENC_API long long beans_enc_b64_decode(unsigned char* src, unsigned char* dst,
    ...
    if (!forgiving && !padded_encoding &&
        outcome.error == simdutf::BASE64_INPUT_REMAINDER && len > 0) {
        size_t padded_len = (len + 3) / 4 * 4;
        char* scratch = (char*)malloc(padded_len);
        if (!scratch) return BEANS_B64_ERR_OTHER;
        memcpy(scratch, src, len);
        for (size_t i = len; i < padded_len; i++) scratch[i] = '=';
        simdutf::base64_options padded =
            encoding == BEANS_B64_STANDARD_NO_PAD
                ? simdutf::base64_default
                : simdutf::base64_url_with_padding;
        outcome = simdutf::base64_to_binary(scratch, padded_len, (char*)dst,
                                            padded, simdutf::strict);
        free(scratch);
        if (outcome.error != simdutf::SUCCESS && outcome.count > len) {
            outcome.count = len;
        }
    }
