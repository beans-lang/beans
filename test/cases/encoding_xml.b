...
fn escaped_owned_node() -> xml.Node {
    let data: Bytes = Bytes.from("<owned><inner>buffer held</inner></owned>")
    match xml.parse_bytes_in_place(move data) {
        ok(doc) => {
            match doc.root() {
                ok(root) => {
                    for child: xml.Node in root.children() { return child }
                }
                err(_) => {}
            }
        }
...
fn main() {
    ...
    let owned_survivor: xml.Node = escaped_owned_node()
    io.println("owned survivor [{owned_survivor.name()}] [{owned_survivor.text()}]")
