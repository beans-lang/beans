...
# The std.os spellings are gone, and the compiler says so.
...
set -e
if [[ "$r1" -eq 0 ]]; then
    echo "std.os still answers to now_ms (selfhost=$r1)" >&2
    exit 1
fi
...
if grep -nE '\b(srand|rand48|mt19937|random\(\))' runtime/beans_rt.c; then
