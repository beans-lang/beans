...
# Context edge sweep:   python3 tools/differential_fuzz.py --edge --cases 50
set -euo pipefail
cd "$(dirname "$0")/.."
...
EOF
    ...
    # Active annotations keep interpreter/native and stage0/self behavior in
    # lockstep, including the nested-dispatch guard.
    cat >"$tmp/runtime_hook.b" <<'EOF'
import std.io

@runtime_hook(before: "before")
@target(value: ["function"])
annotation traced {}

fn before(target: string) {
    io.println("before:{target}")
    nested()
}
...
EOF
    cat >"$tmp/runtime_hook.expected" <<'EOF'
before:main.work
nested
work
EOF
    for cc in build/beansc; do
        "$cc" run "$tmp/runtime_hook.b" >"$tmp/runtime_hook.out"
        cmp -s "$tmp/runtime_hook.expected" "$tmp/runtime_hook.out" || {
            echo "  FAIL: $cc runtime-hook interpreter output" >&2
            return 1
        }
        "$cc" build "$tmp/runtime_hook.b" -o "$tmp/runtime_hook.bin" \
            >/dev/null 2>&1
        "$tmp/runtime_hook.bin" >"$tmp/runtime_hook.out"
        cmp -s "$tmp/runtime_hook.expected" "$tmp/runtime_hook.out" || {
            echo "  FAIL: $cc runtime-hook native output" >&2
            return 1
        }
    done
...
case "$mode" in
    smoke)
        ...
        "$python3" tools/differential_fuzz.py \
            --seed 1 --cases 8 --groups core,classes,annotations --lanes debug \
            --keep-going
        echo "== differential fuzz: packages sweep (seed 1, debug lanes) =="
        "$python3" tools/differential_fuzz.py \
            --seed 1 --cases 6 --groups classes,packages,annotations --lanes debug \
            --keep-going
        echo "== differential fuzz: checker parity (access and ownership) =="
        "$python3" tools/differential_fuzz.py \
            --negative --seed 1 --cases 23 --keep-going
        echo "== differential fuzz: contextual edge cases (debug lanes) =="
        "$python3" tools/differential_fuzz.py \
            --edge --seed 1 --cases 14 --lanes debug --keep-going
    ...
    run)
        ...
        if [ "${FUZZ_NEGATIVE:-1}" = 1 ]; then
            "$python3" tools/differential_fuzz.py \
                ...
                --cases "${FUZZ_NEGATIVE_CASES:-46}" --keep-going
        fi
        if [ "${FUZZ_EDGE:-1}" = 1 ]; then
            "$python3" tools/differential_fuzz.py \
                --edge --seed "${FUZZ_SEED:-1}" \
                --cases "${FUZZ_EDGE_CASES:-28}" \
                --lanes "${FUZZ_LANES:-all}" --keep-going
        fi
        ;;
    *)
