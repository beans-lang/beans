...
if "$BEANSC" build --target $TRIPLE --linker lld compiler/beans/main.b \
    ...
    #
    # ffi.b is skipped for a different and narrower reason: the interpreter
    # compiles a C shim for every extern "C" signature clang has to classify,
    # and for any symbol linked out of a static archive rather than exported by
    # a DLL. Wine has no Windows clang to run, so that whole path is
    # unreachable here — not broken, unreachable. The real-Windows hosted gate
    # has a toolchain and keeps ffi.b with no exemption.
    interp_skip="target_info.b cpu_dispatch.b intrinsics.b poller.b processes.b \
child_process.b signals.b net.b threads.b ffi.b"
    hosted_ran=0
    for src in examples/*.b; do
        name=$(basename "$src")
