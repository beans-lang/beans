...
abi_now=$(grep -oE 'return "enc-abi-[0-9]+"' src/driver.b | head -1)
if [[ -z "$abi_now" ]]; then
    echo "could not find the bridge ABI version in src/driver.b" >&2
    exit 1
fi
mkdir -p "$tmp/abi"
cp -R src "$tmp/abi/src"
sed -i.bak 's/return "enc-abi-[0-9]*"/return "enc-abi-999999"/' \
    "$tmp/abi/src/driver.b"
if ! grep -q 'enc-abi-999999' "$tmp/abi/src/driver.b"; then
    echo "the ABI bump did not apply to the copied compiler source" >&2
    exit 1
fi
if ! ./build/beansc build "$tmp/abi/src/main.b" \
