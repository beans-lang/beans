...
$upgradeInstaller = Join-Path $prefix 'libexec\beans-install.ps1'
$upgradeLauncher = Join-Path $prefix 'libexec\beans-upgrade.ps1'
if (-not (Test-Path -LiteralPath $upgradeInstaller)) {
    Fail 'the Windows package has no checked upgrade installer'
}
if (-not (Test-Path -LiteralPath $upgradeLauncher)) {
    Fail 'the Windows package has no safe upgrade launcher'
}
$launcherText = Get-Content -LiteralPath $launcher -Raw
if ($launcherText -notmatch 'beans-upgrade[.]ps1') {
    Fail 'beansc.cmd does not route beansc upgrade through the safe launcher'
}
Step 'Windows package carries the self-upgrade path'
