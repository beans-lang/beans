...
if [[ "$(uname -s)" == Darwin ]]; then
    ...
    if grep -n 'EV_CLEAR' runtime/beans_rt.c; then
    ...
    if grep -n 'EPOLLET' runtime/beans_rt.c; then
...
# The kind map must carry every slug.
...
if ! BEANS_NO_POOL=1 "$tmp/asan" >"$tmp/asan.out" 2>"$tmp/asan.err"; then
    cat "$tmp/asan.err" >&2
    echo "poller exited non-zero under the sanitizers" >&2
    exit 1
fi
