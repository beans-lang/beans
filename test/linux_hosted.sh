...
run0() { run_qemu "$tmp/beansc0.$arch" "$@"; }

echo "== $arch: build stage 0 from C++ and use its native host default =="
# A hosted self-hosted compiler is not enough for a source checkout: `make`
# starts with beansc0. Cross-building that C++ program and asking it to build
# without --target proves TargetSpec::host() selects this architecture.
if ! make --no-print-directory -s CXX="$cross_cxx" \
        CXXFLAGS="-std=c++20 $host_opt -pthread $host_cxx_flags" \
        LDLIBS="$host_ldlibs" \
        BOOTSTRAP_BIN="$tmp/beansc0.$arch" stage0 >"$tmp/stage0-build.log" 2>&1; then
    echo "  FAIL: could not cross-build beansc0 for $arch"
    tail -8 "$tmp/stage0-build.log"
    exit 1
fi
if ! run0 build "${extra_build_args[@]}" --linker "$linker" \
        examples/hello.b -o "$tmp/stage0-hello" \
        >"$tmp/stage0-hello.log" 2>&1 ||
   [ "$(run_qemu "$tmp/stage0-hello")" != \
     "hello from beans" ]; then
    echo "  FAIL: beansc0 could not build for its default $arch host"
    tail -8 "$tmp/stage0-hello.log"
    exit 1
fi
readelf -h "$tmp/stage0-hello" >"$tmp/stage0-hello.elf"
if ! grep -qi "Machine:.*$elf_machine" "$tmp/stage0-hello.elf"; then
    echo "  FAIL: beansc0's no --target build was not a $elf_machine binary"
    cat "$tmp/stage0-hello.elf"
    exit 1
fi
if [ "$arch" = "riscv64" ]; then
    cat >"$tmp/stage0-cpu.b" <<'EOF'
import std.io
import std.cpu
fn main() {
...
EOF
    run0 run "$tmp/stage0-cpu.b" >"$tmp/stage0-cpu.out" 2>&1
    BEANS_CPU_FEATURES= run0 run "$tmp/stage0-cpu.b" \
        >"$tmp/stage0-cpu.masked" 2>&1
    if [ "$(grep -c ' true$' "$tmp/stage0-cpu.out")" != "5" ] ||
       [ "$(grep -c ' false$' "$tmp/stage0-cpu.masked")" != "5" ]; then
        echo "  FAIL: beansc0's RISC-V CPU detector is wrong"
        cat "$tmp/stage0-cpu.out" "$tmp/stage0-cpu.masked"
        exit 1
    fi
fi
echo "  ok: beansc0 selects $triple and builds with no --target"

echo "== $arch: cross-build beansc for the target =="
if ! "$beansc" build --target "$triple" "${extra_build_args[@]}" \
        --linker "$linker" compiler/beans/main.b \
...
if run llvm compiler/beans/main.b >"$tmp/stage1.ll" 2>"$tmp/s1.err" \
   && run build "${extra_build_args[@]}" --linker "$linker" \
        compiler/beans/main.b -o "$tmp/beansc-stage2.$arch" >"$tmp/s2.err" 2>&1; then
    run_qemu "$tmp/beansc-stage2.$arch" llvm compiler/beans/main.b \
...
echo "   self-compile fixed point, and drives $ran examples to byte-identical output"
