...
grep -q '^fn main::choose : fn(bool) -> int$' "$out"
grep -q '^fn main::select : fn(bool) -> int$' "$out"
grep -q '^fn main::branch_owned : fn(bool) -> string$' "$out"
grep -q '^fn main::formatted : fn(float) -> string$' "$out"
grep -q '^fn main::interpolation_closure : fn(int) -> string$' "$out"
grep -q '^fn main::temporary_order : fn(bool) -> int$' "$out"
grep -q '^fn main::lazy : fn(bool) -> bool$' "$out"
grep -q '^fn main::make_adder : fn(int) -> fn(int) -> int$' "$out"
grep -q '^fn main::make_nested : fn(int) -> fn() -> fn() -> int$' "$out"
grep -q '^closure#[0-9][0-9]* @[0-9][0-9]*:[0-9][0-9]* : fn(int) -> int$' "$out"
grep -q '^fn main::unwrap : fn(Option<int>) -> int$' "$out"
grep -q '^fn main::move_then_replace : fn() -> string$' "$out"
grep -q '^fn main::deferred : fn(Result<int, Error>) -> Result<int, Error>$' "$out"
grep -q '^fn main::deferred_capture : fn(string)$' "$out"
grep -q '^fn main::build_defaults : fn()$' "$out"
grep -q '^fn main::loop_owner : fn(List<string>, int) -> int$' "$out"
grep -q '^fn main::walk : fn(int) -> int$' "$out"
grep -q '^fn main::optimized_value : fn() -> int$' "$out"
grep -q '^fn main::escaping_value : fn(string) -> string$' "$out"
grep -q '^fn main::scalar_partial_escape : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_no_escape : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_default_order : fn() -> int$' "$out"
grep -q '^fn main::scalar_fixed_array : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_deinit_fallback : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_effect_fallback : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_owned_field_fallback : fn() -> int$' "$out"
grep -q '^fn main::scalar_write_fallback : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_identity_fallback : fn(int) -> bool$' "$out"
grep -q '^fn main::scalar_capture_fallback : fn(int) -> int$' "$out"
grep -q '^fn main::scalar_two_escapes_fallback : fn(int) -> int$' "$out"
grep -q '^fn main::main : fn()$' "$out"
...
grep -q \
    ...
    "$out"
grep -q \
    'aggregate=new positional=(v[0-9][0-9]*) passing=(borrow) consumes=(0) name=main::OwnedSink' \
    "$out"
test "$(
    grep -c \
...
' "$out"
grep -q \
    'local .* alias : main.Item .* borrows=l[0-9][0-9]* scalar-replaced' \
    "$out"
test "$(
...
grep -q 'local l0 self : main.DeferredSelf borrowed capture' "$out"
grep -q 'interpolation=(v[0-9][0-9]*:8.2)' "$out"
grep -q ' releases=(v[0-9][0-9]*)' "$out"
grep -q -- '-> .* releases=(v[0-9][0-9]*)' "$out"
awk '
    /^fn main::temporary_order / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
...
awk '
    /^fn main::move_then_replace / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
...
' "$out"
test "$(grep -c 'name=main::default_label$' "$out")" -ge 2
awk '
    /^fn main::loop_owner / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
...
' "$out"
awk '
    /^fn main::optimized_value / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
...
' "$out"
awk '
    /^fn main::escaping_value / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
...
' "$out"
awk '
    /^fn main::scalar_partial_escape / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
    in_fn && /local .* item : main.Item .* scalar-replaced/ {
        item = 1
    }
    in_fn && /local .* alias : main.Item .* scalar-replaced/ {
        alias = 1
    }
...
' "$out"
awk '
    /^fn main::scalar_no_escape / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
    in_fn && /local .* item : main.Item .* scalar-replaced/ {
        replaced = 1
    }
...
' "$out"
awk '
    /^fn main::scalar_default_order / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
    in_fn && /local .* item : main.DefaultScalar .* scalar-replaced/ {
        replaced = 1
    }
    END { exit !replaced }
' "$out"
awk '
    /^fn main::scalar_fixed_array / { in_fn = 1; next }
    in_fn && /^fn / { in_fn = 0 }
    in_fn && /local .* item : main.FixedScalar .* scalar-replaced/ {
        replaced = 1
    }
    END { exit !replaced }
' "$out"
...
grep -q 'define void @m_main$OwnedSink_init[$]owned1' \
    build/mir_control.ll
if grep -q 'define void @m_main[$]\\(ConditionalSink\\|DoubleSink\\|OverwriteSink\\)_init[$]owned' \
    build/mir_control.ll; then
    echo "unsafe ownership sink variant was emitted" >&2
    exit 1
fi
sed -n \
    '/^define i64 @b_main$scalar_no_escape(/,/^}/p' \
...
test "$(
    sed -n \
        '/^define i64 @b_main$scalar_partial_escape(/,/^}/p' \
        build/mir_control.ll |
        grep -c 'call ptr @beans_alloc'
)" -eq 1
grep -q 'call ptr @beans_alloc' < <(
    sed -n \
        '/^define i64 @b_main$scalar_deinit_fallback(/,/^}/p' \
        build/mir_control.ll
)
awk '
    /^define i64 @b_main[$]scalar_default_order\(/ { in_fn = 1; next }
    in_fn && /^}/ { in_fn = 0 }
    in_fn && /call i64 @b_main[$]scalar_arg\(/ { arg_line = NR }
    in_fn && /call i64 @b_main[$]scalar_default\(/ { default_line = NR }
    END {
...
' build/mir_control-cfg.ll
sed -n \
    '/^define i64 @b_main$map_accumulate(/,/^}/p' \
