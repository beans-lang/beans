...
mkdir -p "$tmp/project/examples/multi" "$tmp/project/tests" \
    "$tmp/project/tools"
cat >"$tmp/project/examples/hello.b" <<'BEANS'
package main

import std.io
import beans_test_library

fn main() { io.println(api.doubled(21)) }
BEANS
cat >"$tmp/project/tests/api_test.b" <<'BEANS'
package main

import std.io
import beans_test_library

fn main() { io.println(api.doubled(20) + 2) }
BEANS
cat >"$tmp/project/examples/multi/main.b" <<'BEANS'
package main

import std.io
import beans_test_library

fn main() { io.println(tripled(api.doubled(7))) }
BEANS
cat >"$tmp/project/examples/multi/helper.b" <<'BEANS'
package main

fn tripled(value: i32) -> i32 { return value * 3 }
BEANS
cat >"$tmp/project/tools/not_an_entry.b" <<'BEANS'
package main
fn main() {}
BEANS

library_compilers=("$beansc")
for library_compiler in "${library_compilers[@]}"; do
    test "$("$library_compiler" run "$tmp/project/examples/hello.b")" = "42"
    test "$("$library_compiler" run "$tmp/project/tests/api_test.b")" = "42"
    test "$("$library_compiler" run "$tmp/project/examples/multi/main.b")" = "42"
    test "$(cd "$tmp/project/examples/multi" &&
        "$library_compiler" run main.b)" = "42"
    "$library_compiler" build "$tmp/project/examples/hello.b" \
        -o "$tmp/example-native" >"$tmp/example.build"
    test "$("$tmp/example-native")" = "42"
    if "$library_compiler" run "$tmp/project/tools/not_an_entry.b" \
        >"$tmp/tools.out" 2>&1; then
        echo "an arbitrary nested library package ran as an application" >&2
        exit 1
    fi
    grep -F 'entry file must sit next to beans.pot' "$tmp/tools.out" \
        >"$tmp/match"
done
