...
# One table, two readers: the compiler's own level function and the runtime's
# defines. If the numbers drift, the failure is a link error naming a mangled
# symbol rather than anything about capabilities.
grep -q 'if profile == "freestanding" { return 1 }' src/hir.b
grep -q 'if profile == "minimal" { return 2 }' src/hir.b
grep -A4 'fn runtime_level(profile: string) -> int {' src/hir.b | grep -q 'return 3'
...
while IFS= read -r path; do
    ...
    if [[ ! -d "stdlib/std/$module_path" ]] && ! grep -q "^import $path " src/interpreter.b; then
        echo "the capability table names '$path', which is neither a shipped package" \
             "nor a native module" >&2
        exit 1
    fi
done < <(grep -oE '"std\.[a-z]+"' src/hir.b | tr -d '"' | sort -u)
