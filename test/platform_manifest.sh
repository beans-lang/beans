...
if ! diff -u "$tmp/beansc0.all" "$tmp/beansc.all"; then
    echo "stage 0 and the self-hosted compiler register different targets" >&2
    exit 1
fi

while read -r triple; do
    os=$(./build/beansc0 target "$triple" | awk '/^os / { print $2 }')
    if [[ "$os" == linux || "$os" == windows ]]; then
        printf '%s\n' "$triple"
    fi
done <"$tmp/beansc0.all" | sort >"$tmp/compiler-platforms"
