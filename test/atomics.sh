...
# The final `1` is MemoryOrder.acquire. It must reach the C runtime and the
# interpreter's own re-read rather than being accepted and discarded.
grep -q 'beans_ordered_load(address, width, order)' runtime/beans_rt.c
grep -q 'park_wait(cell.v, expected, budget, bounded, order)' compiler/bootstrap/interp.cpp
