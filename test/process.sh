...
# The count has to be scoped by parent and taken while the program is still
# alive, or it cannot see the thing it is looking for. A machine-wide count
# charges this test for any unrelated defunct process the host is carrying, and
# a desktop can carry one for days; a count taken after the program exits sees
# nothing either way, because a leaked child is reparented to init the moment
# its parent dies and init reaps it. So the program holds itself open once the
# fifty runs are done, and the count asks only for children of that pid.
cat >"$tmp/many.b" <<'MANY'
import std.io
import std.process
fn main() {
    ...
    // The hold. It is a live child, never a defunct one, so it cannot be
    // mistaken for a leak by the count it exists to make possible.
    let hold: process.Command = new process.Command("/bin/sleep")
    hold.arg("5")
    match hold.run() { ok(done) => {} err(error) => {} }
}
MANY
./build/beansc build "$tmp/many.b" -o "$tmp/many" >/dev/null 2>&1
"$tmp/many" >"$tmp/many.out" 2>&1 &
many_pid=$!
# The hold appearing as a live child is the signal that the fifty runs are done.
# Waiting on the stdout marker instead would depend on when the runtime flushes
# a redirected stream, which is not what this check is about. Same 120s bound
# the rest of the file uses.
held=""
for _ in $(seq 1 1200); do
    if ps -ax -o ppid=,comm= | awk -v p="$many_pid" '$1 == p' | grep -q sleep; then
        held=yes
        break
    fi
...
if [[ -z "$held" ]]; then
    echo "the fifty runs never reached the hold" >&2
    wait "$many_pid" || true
    exit 1
fi
...
# Large payloads stay in their original stdout/stderr buffers. The runtime no
# longer joins them into one packed Bytes for the stdlib to slice apart.
if grep -q '24 + out->len + err->len' runtime/beans_rt.c; then
