...
if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    DYLD_INSERT_LIBRARIES="$tmp/layout.dylib" \
        "$beansc" run test/cases/c_layout_c_abi.b >"$tmp/interp"
else
...
# binary explicitly.
"$beansc" build test/cases/c_layout_c_abi.b -o "$tmp/unlinked" \
...
clang -O2 -pthread -Wno-override-module build/c_layout_c_abi.ll \
    ...
    test/fixtures/c_layout_helper.c -lm -o "$tmp/native"
...
clang -O1 -g -pthread -fsanitize=address,undefined \
    ...
    test/fixtures/c_layout_helper.c -lm -o "$tmp/asan"
