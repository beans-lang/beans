...
# VS Code rejects the whole reply when a name's selection range falls outside
# its declaration range. Fields used to have a one-character declaration
# range, so every field name longer than one character broke the outline.
def check_symbol_ranges(items, parent=""):
    for item in items:
        full = item["range"]
        selected = item["selectionRange"]
        lo = (full["start"]["line"], full["start"]["character"])
        hi = (full["end"]["line"], full["end"]["character"])
        selected_lo = (selected["start"]["line"],
                       selected["start"]["character"])
        selected_hi = (selected["end"]["line"],
                       selected["end"]["character"])
        if not lo <= selected_lo <= selected_hi <= hi:
            fail(f"document symbol {parent}{item['name']} has selection "
                 f"{selected_lo}..{selected_hi} outside {lo}..{hi}")
        check_symbol_ranges(item.get("children", []),
                            parent + item["name"] + ".")

check_symbol_ranges(s.result(following))
