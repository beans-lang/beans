...
./build/beansc mir examples/fixed_arrays.b >"$tmp/fixed.mir"
test "$(grep -c 'iterate_init.*borrow-elided' "$tmp/fixed.mir")" -eq 1
test "$(grep -c 'spill.iter.array.* = alloca' build/fixed_arrays.ll)" -eq 1
