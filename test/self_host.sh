...
cd "$(dirname "$0")/.."
reference_compiler="${BEANSC:-$PWD/build/beansc}"
...
if [[ "$(uname -s)" == "Darwin" ]]; then
    preload_name=DYLD_INSERT_LIBRARIES
    clang -O2 -fvisibility=hidden -dynamiclib \
        runtime/encoding/beans_enc_json.c \
        -o "$tmp/self_host_json.dylib"
    clang -x c++ -std=c++17 -fno-exceptions -fno-rtti -O2 \
        -fvisibility=hidden -dynamiclib \
        runtime/encoding/beans_enc_xml.cpp \
        -o "$tmp/self_host_xml.dylib"
    self_host_json_library="$tmp/self_host_json.dylib"
    self_host_xml_library="$tmp/self_host_xml.dylib"
else
    preload_name=LD_PRELOAD
    clang -O2 -fvisibility=hidden -shared -fPIC \
        runtime/encoding/beans_enc_json.c \
        -o "$tmp/self_host_json.so"
    clang -x c++ -std=c++17 -fno-exceptions -fno-rtti -O2 \
        -fvisibility=hidden -shared -fPIC \
        runtime/encoding/beans_enc_xml.cpp \
        -o "$tmp/self_host_xml.so"
...
for interpreted in examples/*.b examples/shop/main.b; do
    case "$interpreted" in
        examples/zero_copy_json.b)
            compare_interpreters_with_preload \
                "$interpreted" 0 "$self_host_json_library"
            ;;
        examples/zero_copy_xml.b)
            compare_interpreters_with_preload \
                "$interpreted" 0 "$self_host_xml_library"
            ;;
...
grep -q '^link manifest run/build and dependency propagation ok$' \
    "$tmp/self-host-link-manifest.out"
...
./build/beansc-next load src/main.b >"$tmp/loader.first"
./build/beansc-next load src/main.b >"$tmp/loader.second"
...
./build/beansc-next hir src/main.b >"$tmp/hir.first"
./build/beansc-next hir src/main.b >"$tmp/hir.second"
...
./build/beansc-next mir src/main.b >"$tmp/compiler.mir"
...
for example_path in examples/*.b; do
    ...
    if [[ -n "$example_preload" ]]; then
        env "$preload_name=$example_preload" \
...
./build/beansc-next check src/main.b \
...
grep -q "'one' takes 1 argument but got 2" \
    "$tmp/expressions.bad"
...
grep -q "map index assignment only supports '='" \
    "$tmp/index_compound_bad.out"
grep -q "init runs when the object is built" \
    "$tmp/lifecycle_direct_call_bad.out"
grep -q "deinit runs by itself when the last reference drops" \
    "$tmp/lifecycle_direct_call_bad.out"
...
grep -q 'List needs 1 type argument but got 0' \
...
(
    cd "$tmp/dependency/app"
    BEANS_HOME="$tmp/dependency/home" "$next_compiler" pot tidy
) >/dev/null
...
(
    ...
    BEANS_HOME="$tmp/dependency/home" GIT_ALLOW_PROTOCOL=file \
        ...
        GIT_CONFIG_KEY_0="url.file://$tmp/dependency/remotes/.insteadOf" \
        GIT_CONFIG_VALUE_0="https://example.test/" \
        "$next_compiler" pot update example.test/acme/dep
...
if (
    cd "$tmp/dependency/app"
    BEANS_HOME="$tmp/dependency/home" \
        "$next_compiler" pot update example.test/acme/unknown
...
./build/beansc-next lex src/main.b >"$tmp/compiler.first"
./build/beansc-next lex src/main.b >"$tmp/compiler.second"
cmp "$tmp/compiler.first" "$tmp/compiler.second"
./build/beansc-next parse src/parser.b >"$tmp/parser.first"
./build/beansc-next parse src/parser.b >"$tmp/parser.second"
...
for source in examples/hello.b examples/tour.b examples/clones.b src/*.b; do
...
while IFS= read -r source; do
    ...
    if "$reference_compiler" parse "$source" \
        ...
        if ! ./build/beansc-next parse "$source" \
            ...
            echo "Beans parser rejected a reference-accepted source: $source" >&2
        ...
        if ./build/beansc-next parse "$source" \
            >"$tmp/next-$parsed.ast" \
            2>"$tmp/next-$parsed.err"; then
            echo "Beans parser accepted a reference-rejected source: $source" >&2
            exit 1
        fi
        cmp "$tmp/reference-$parsed.ast" \
            "$tmp/next-$parsed.ast"
    ...
    find examples src stdlib bench test/cases \
        -name '*.b' -type f | sort
)
