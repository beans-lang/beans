...
# the two compilers already anchor a "has no method" error differently (stage 0
# at the call's paren, the self-hosted one at the name), which predates this
# file — what has to match here is which name each refuses and how it says so.
...
both_reject_same() { # <source> <required message fragment>
    ...
    if [ "$r0" -eq 0 ] || [ "$r1" -eq 0 ]; then
        echo "api_names: $src accepted (stage0=$r0 selfhost=$r1)" >&2
        cat "$tmp/a0" "$tmp/a1" >&2
        exit 1
    fi
...
# A static that no longer exists on a *builtin* type is refused by both
# compilers, but not in the same words: stage 0 names the missing static, the
# self-hosted compiler reports the owner as an unknown name. That split predates
# this file, so both wordings are pinned rather than diffed — either one drifting
# still fails here.
reject_builtin_static() { # <label> <owner> <name> <body...>
    ...
    grep -q "$owner has no static '$name'" "$tmp/a0" || {
        echo "api_names: stage 0 refused $src with the wrong message:" >&2
        cat "$tmp/a0" >&2
        exit 1
    }
...
reject() {
    ...
    shift 2
    printf '%s\n' "$@" >"$tmp/rej_$label.b"
    both_reject_same "$tmp/rej_$label.b" "$fragment"
}

echo "checking every renamed public name is gone from both compilers"
...
"$BEANSC0" run "$tmp/renamed.b" >"$tmp/renamed.stage0"
"$BEANSC" build "$tmp/renamed.b" -o "$tmp/renamed" >/dev/null
"$tmp/renamed" >"$tmp/renamed.native"
diff -u "$tmp/renamed.stage0" "$tmp/renamed.selfhost"
diff -u "$tmp/renamed.stage0" "$tmp/renamed.native"
diff -u - "$tmp/renamed.stage0" <<'EXPECTED'
