...
fn is_absolute(value: string) -> bool {
    if value.is_empty() { return false }
    if value.byte_at(0) == 47 { return true }
    if value.len() < 3 || value.byte_at(1) != 58 ||
       value.byte_at(2) != 47 {
        return false
    }
    let drive: int = value.byte_at(0)
    return (drive >= 65 && drive <= 90) ||
           (drive >= 97 && drive <= 122)
}
...
/// `/` or a Windows drive such as `C:/`) it is returned as-is; empty segments
/// are skipped.
pub fn join(first: string, second: string) -> string {
    if is_absolute(second) { return second }
