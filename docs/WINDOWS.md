...
Every row has a CI job that runs programs, runs `beansc.exe`, rebuilds the
compiler to a fixed point, and bootstraps from the C++ source compiler. Decimal
uses portable two-limb arithmetic, so i686 does not need C++ `__int128`.
...
- `windows-native` — runs each bundle on a real Windows machine.
  ...
  It also builds the C++ stage 0 from source and requires stage 2 and stage 3
  to be byte-identical.
- `windows-msvc` — repeats the program, hosted compiler, fixed-point and C++
  source-bootstrap gates for x86-64, i686 and ARM64 with MSVC libraries.
