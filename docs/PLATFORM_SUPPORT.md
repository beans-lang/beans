...
3. A clean checkout can build the C++ stage 0 and reach the same fixed point.
...
- [x] Check that stage 0, the self-hosted compiler and the manifest register the
  same Windows/Linux targets.
- [x] Track program, compiler, source-bootstrap and archive gates separately.
...
- [x] Move integer and decimal literal parsing in `compiler/bootstrap/checker.cpp` onto those
  types, including exact overflow messages.
- [x] Move `Decimal`, constant folding and rounding in the C++ interpreter and
...
- [x] Change the generated-code/runtime decimal boundary so no C declaration
  ...
  already does.
- [x] Port the same representation and operations through `compiler/beans/*.b`. LLVM
...
- [x] Bump the runtime ABI and pass the final bootstrap, ASan, UBSan, TSan and
  leak gates after the whole branch is assembled.

Exit gate: the compiler and runtime build with a 32-bit C/C++ toolchain and the
`has_int128` target refusal is no longer needed for `decimal`.

### 2. Promote the three registered 32-bit targets

- [x] Add i686 Linux and ARMv7 Linux to stage-0 host detection.
- [x] Run their stage 0, compiler fixed point and differential loop under QEMU.
...
### 3. Make Windows bootstrap from source

- [x] Put C++ process creation behind one host abstraction. Keep
  `posix_spawnp` on Unix and use `CreateProcessW` with exact Windows quoting.
- [x] Port temporary files, dynamic-library lookup and interpreter C-bridge
  loading used by stage 0.
- [x] Add a native Windows build entry point for the C++20 compiler.
- [ ] On x64, ARM64 and x86 Windows, run stage 0 -> stage 1 -> stage 2 -> stage 3
  and require stages 2 and 3 to be byte-identical.
...
- [x] Add full source-bootstrap and hosted gates in Alpine/QEMU environments.
...
- [ ] Run the source-bootstrap and release jobs for GNU/GNullVM/MSVC x86, x64
...
- [x] Add and run the LoongArch64 musl source-bootstrap gate.
- [x] Add ARMv6 soft-float and hard-float as distinct ABI targets.
- [x] Run compiler bootstrap gates on real architecture code through QEMU.
