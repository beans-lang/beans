...
Toolchain status: **self-hosted lexer + parser + loader + checker + MIR + interpreter + native backend implemented** — including multi-file modules, git imports, and selectable build targets. The default `beansc` is written in Beans; the C++ stage-0 bootstrap remains available as `beansc0`.
...
need `fn main()` when built as a binary or run. Libraries reject `main`; their
root package exposes source APIs with `pub`, and native C APIs with
`pub extern "C"`.
...
- A git import needs `host/owner/repo`; the repo must carry its own `beans.pot`.
  `beansc mod tidy` resolves it and writes `beans.lock`. Each lock row stores the
  module path, requested reference, exact commit, and Git tree hash. Resolved
  trees live under `$BEANS_HOME/pkg/<module>/<commit>`.
- `beansc mod update [module]` refreshes all dependencies or one named module.
...
hosting the compiler. The hosted gate also cross-builds the C++ `beansc0`, checks
that its no-`--target` default selects the real host, and uses it to build a program;
that is what proves a source checkout can bootstrap there.
...
cannot leak into a musl build. CI performs a clean C++ bootstrap, self-hosted
...
- **Every row has a native Windows source-bootstrap and hosted job.** The job
  builds C++ stage 0, reaches a byte-identical stage-2/stage-3 fixed point, then
  makes the hosted compiler build and run `hello`. The current proof state is
...
only a template the selected architecture has a row for in `compiler/bootstrap/asm_ops.h`, and the
constraint string has to be the row's too.
